...
//-------------------------------------------------------------------------//
profiler::profiler(onevent_t fonevent /*= {}*/) : begin(std::chrono::steady_clock::now()), onevent(std::move(fonevent))
{
}
...
profiler::~profiler() noexcept
{
  try
  {
    ...
    else
    {
      ...
      profiler::event_default(
          duration_t(std::chrono::steady_clock::now() - this->begin));
    }
  }
  catch (...)
  {
