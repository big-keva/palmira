...
public:
  ...
  {
    auto push_module = [&](const std::string &module_name,
                           const mtc::zmap &settings) -> void
    {
      // Loading a module.
      ...
      if (handle != nullptr)
      {
        ...
        if (error == nullptr)
        { //<TODO> Adding error handle.
        }

        auto module = std::make_shared<module_file>(
            module_file{.settings = settings, .handle = handle});
    ...
    if (config.has_key("path") && not config.get_charstr("path").empty())
    { //<TODO> Add impl settings
      if (not std::filesystem::is_directory(config.get_charstr("path")))
      {
        throw(std::invalid_argument("Module path is not directory: " +
                                    config.get_charstr("path")));
      }

      for (const auto &module :
           std::filesystem::directory_iterator(config.get_charstr("path")))
      {
    ...
    if (config.has_key("files") &&
        not config.to_zmap().get_array_zmap("files", {}).empty())
    {
      for (const auto &module_settings :
           config.to_zmap().get_array_zmap("files", {}))
      {
        const auto file_name = module_settings.get_charstr("file", "");
        if (not file_name.empty() &&
            not std::filesystem::is_regular_file(file_name))
        {
  ...
  auto
  load(const char *symbol_name,
       std::function<void(symbol_t symbol, const mtc::zmap &module_settings,
                          const char *error)>
           onload) const -> void
  {
    for (const auto &plugin : this->plugins)
    {
      ...
      auto symbol =
          reinterpret_cast<symbol_t>(dlsym(plugin.second->handle, symbol_name));

      if (onload != nullptr)
      {
        // Raising ONLOAD event.
        onload(reinterpret_cast<symbol_t>(symbol), plugin.second->settings,
               dlerror());
      }
    }
  }
