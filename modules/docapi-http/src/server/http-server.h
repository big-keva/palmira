...
auto HttpServer::onpost(Response *res, Request *req) -> void
{
  ...
  {
    const auto timeout =
        this->settings.get_int32("request_timeout", request_timeout);
    ...
    this->executer.enqueue(executer_types::insert, [loop = this->loop, service = this->service, resp_ctx, index, id, body = std::move(*body), params, timeout, &state, started](const executer_context &ctx) mutable -> void
      {
        try
        {
          ...
          auto args = json::elastic::parse_index_request(std::string_view(body), mtc::zmap{{"_index", index}, {"_id", id}, {"_params", params}, {"_started", started}});
...
auto HttpServer::onget(Response *res, Request *req) -> void
{
  ...
  res->onData([this, resp_ctx, index, id, params, routing, state, body, body_size, request_started](std::string_view chunk, bool last) mutable
  {
    ...
    if (body->size() + chunk.size() > body_size)
    {
      ...
      // Replying error message.
      http::send_error_response(
          resp_ctx.get(), http::status_codes::PAYLOAD_TOO_LARGE,
          "payload_too_large", "request body is too large");
      return;
    }
    body->append(chunk.data(), chunk.size());
                           ...
                           [service = this->service, resp_ctx, index, id, params, routing, body = std::move(*body), started = request_started.time_since_epoch().count(), state, loop = uWS::Loop::get()](const executer_context &ctx) mutable
    {
      try
      {
        ...
        const auto req =
            json::elastic::parse_search_request(body, {index});

        // Setting an order of searching.
        args.order =
            mtc::zmap{{"first", req.from}, {"count", req.size}};

        args.query = mtc::zmap{{"id", id}};
        ...
        service->Search(args, [resp_ctx, index, ctx, state, loop](const mtc::zmap &resp)
        {
          ...
          // Making a response.
          reply.body = http::elastic::make_search_response(mtc::zmap{{"resp", resp}, {"_index", index}});
