...
auto detect_query_kind(simdjson::ondemand::object query_object) -> query_kinds
{
  ...
  for (auto field_result : query_object)
  {
    ...
    throw_if_error(std::move(field_result).get(field),
                   "failed to read query field");

    const std::string_view key = read_key(field);
    if (key == "match_all")
    {
      return query_kinds::match_all;
    }
    if (key == "match_none")
    {
      return query_kinds::match_none;
    }
    if (key == "term")
    {
      return query_kinds::term;
    }
    if (key == "terms")
    {
      return query_kinds::terms;
    }
    if (key == "range")
    {
      return query_kinds::range;
    }
    if (key == "bool")
    {
      return query_kinds::bool_query;
    }
    if (key == "as_tree")
    {
      return query_kinds::as_tree;
    }
...
auto read_string_view(simdjson::ondemand::value value, std::string_view context)
    -> std::string_view
{
...
auto read_scalar_as_string(simdjson::ondemand::value value,
...
{
  std::string_view key;
  throw_if_error(field.unescaped_key(false).get(key),
                 "failed to read object key");

  return key;
}

auto read_double(simdjson::ondemand::value value, std::string_view context)
    -> double
{
  double result = 0.0;
...
auto read_double_value(simdjson::ondemand::value value,
...
auto read_string_value(simdjson::ondemand::value value,
                       std::string_view context) -> std::string
{
  std::string_view result;
  throw_if_error(value.get_string().get(result), context);
...
std::uint64_t read_non_negative_integer(simdjson::ondemand::value value,
                                        std::string_view field_name)
{
  simdjson::ondemand::number number;
  throw_if_error(value.get_number().get(number),
                 std::string("failed to read number field: ") +
...
{
  bool result = false;
  throw_if_error(value.get_bool().get(result),
                 std::string("failed to read bool field: ") +
                     std::string(field_name));

  return result;
}

auto read_bool_value(simdjson::ondemand::value value, std::string_view context)
    -> bool
{
  bool result = false;
...
void parse_string_array(simdjson::ondemand::array array,
