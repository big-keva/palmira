  ...
  for (auto field_result : term_object)
  {
    ...
    if (not object_result.error())
    {
      ...
      for (auto param_result : params)
      {
        ...
        {
          result.boost =
              read_double_value(param_value, "term.boost must be double");
        }
      }
    }
    else
...
auto parse_terms_query(simdjson::ondemand::object terms_object) -> terms_query
{
  ...
  for (auto field_result : terms_object)
  {
    simdjson::ondemand::field field;
    throw_if_error(std::move(field_result).get(field),
                   "failed to read terms field");

    result.field = copy_string(read_key(field));

    simdjson::ondemand::array values;
    throw_if_error(field.value().get_array().get(values),
                   "terms query value must be array");

    for (auto value_result : values)
    {
      simdjson::ondemand::value value;
      throw_if_error(std::move(value_result).get(value),
                     "failed to read terms value");
