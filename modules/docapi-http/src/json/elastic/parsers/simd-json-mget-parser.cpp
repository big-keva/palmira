...
{
//-------------------------------------------------------------------------//
mget_doc parse_mget_doc_object(simdjson::ondemand::object object,
...
//-------------------------------------------------------------------------//
mget_request parse_mget_request(std::string_view body,
                                std::string_view default_index)
{
  ...
  for (auto field_result : root)
  {
    simdjson::ondemand::field field;
    throw_if_error(std::move(field_result).get(field),
                   "failed to read mget root field");
    ...
    if (key == "docs")
    {
      ...
      for (auto doc_result : docs)
      {
        ...
        throw_if_error(std::move(doc_result).get(doc_value),
                       "failed to read mget docs item");

        simdjson::ondemand::object doc_object;
        throw_if_error(doc_value.get_object().get(doc_object),
                       "mget docs item must be object");

        request.docs.emplace_back(
            parse_mget_doc_object(doc_object, default_index));
      }
    }
    else if (key == "ids")
    {
      ...
      for (auto id_result : ids)
      {
        ...
        throw_if_error(std::move(id_result).get(id_value),
                       "failed to read mget id");
