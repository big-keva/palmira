...
//-------------------------------------------------------------------------//
  auto CreateServer(mtc::api<palmira::IService> service, const mtc::config &config) -> mtc::api<palmira::IServer>
  {
    ...
    // Loading a plugin.
    return palmira::LoadPlugin<mtc::api<palmira::IServer>(mtc::api<palmira::IService>, const mtc::config &),
                               mtc::api<palmira::IService>,
                               mtc::config>(plugin_path,
                                       "createServer",
                                            std::move(service),
                                            mtc::config(config));
  }
