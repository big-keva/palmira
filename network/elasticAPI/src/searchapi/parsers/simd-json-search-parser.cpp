...
//-------------------------------------------------------------------------//
    ...
    auto parse_sort(simdjson::ondemand::value value, search_request &request) -> void
    {
      ...
      for (auto item_result : array)
      {
        ...
        {
          request.sort.push_back(sort_clause{.field = copy_string(sort_string), .order = "asc"});
          continue;
        }
    ...
    auto parse_docvalue_fields(simdjson::ondemand::value value, search_request &request) -> void
    {
      ...
      for (auto item_result : array)
      {
        ...
        {
          request.docvalue_fields.push_back(field_request{.field = copy_string(string_item), .format = ""});
          continue;
        }
...
//-------------------------------------------------------------------------//
  search_request parse_search_request(std::string_view body, std::vector<std::string> indices)
  {
    ...
    for (auto field_result : root)
    {
      ...
      {
        parse_string_array(value, "stored_fields must be array", request.stored_fields);
      }
      else if (key == "fields")
      {
        parse_string_array(value, "fields must be array", request.fields);
      }
      ...
      {
        parse_string_array(value, "search_after must be array", request.search_after);
      }
      ...
      else if (key == "query")
      {
        ...
        // Parsing a query.
        request.query = parse_query_object(query_object);
      }
    }
